...
const SHORT_PARAM_SINGLE_VALIDATION = [
    {
        name: 'id',
        isRequired: true,
...
const SHORT_BODY_VALIDATION = [
    ...
    },
    {
        name: 'stock_code',
        isRequired: true,
        rule: (stock_code) => stock_code.toString().length === 5,
        errorMessage: 'Stock Code is must be a number and is required"'
    },
    {
        name: 'reporting_date',
        isRequired: false,
        rule: (reporting_date) => stringToDateConverter(reporting_date) !== null,
        errorMessage: 'Reporting Date must be formatted like so: yyyy-MM-dd'
    },
    {
        name: 'shorted_shares',
        isRequired: false,
        rule: (shorted_shares) => !isNaN(Number(shorted_shares)),
        errorMessage: 'Shorted Shares must be a number'
    },
    {
        name: 'shorted_amount',
        isRequired: false,
        rule: (shorted_amount) => !isNaN(Number(shorted_amount)),
...
const getShortData = async (args) => {
    ...
    let validationResult = validator(args, SHORT_PARAM_VALIDATION);
...
const postShortData = async (data) => {
    console.log(data);
    //TODO: if stockCode is invalid/does not exist, return error
    let validationResult = validator(data, SHORT_BODY_VALIDATION);
    if (validationResult.length > 0)
        throw new InvalidRequestError(validationResult);
    let result = [];
...
const getShortDatum = async (args) => {
    let validationResults = validator(args, SHORT_PARAM_SINGLE_VALIDATION);
    if (validationResults.length > 0)
        throw new InvalidRequestError(validationResults);
    //TODO: if stockCode is invalid/does not exist, return error
    let result = [];
    try {
        result = await executeQuery({
...
const putShortDatum = async (data) => {
    let validationResults = validator(data, SHORT_BODY_VALIDATION);
    if (validationResults.length > 0)
        throw new InvalidRequestError(validationResults);
    //TODO: if stockCode is invalid/does not exist, return error
    let result = [];
...
const deleteShortDatum = async (args) => {
    let validationResults = validator(args, SHORT_PARAM_SINGLE_VALIDATION);
    if (validationResults.length > 0)
        throw new InvalidRequestError(validationResults);
    try {
...
export { getShortData, postShortData, getShortDatum, putShortDatum, deleteShortDatum, retrieveShortDataFromSource };
